...
COMPILE_COMMAND = ["kusion", "compile", "-Y", "ci-test/settings.yaml,kcl.yaml", "-o", "ci-test/stdout.golden.yaml"]
...
def pretty_print(app_path: str):
    """命令行可视化输出"""
    print(f"{GREEN}{app_path}{RESET}", f"{BOLD}{GREEN}[ALL DONE]{RESET}".rjust(TERMINAL_WIDTH * 2 // 3 - len(app_path)), flush=True)
    ...
    for i, env_path in enumerate(env_paths):
        ...
        if envpath_to_result[env_path]:
            print(f"{GREEN}{env_info}{RESET}", f"{BOLD}{GREEN}[Success]{RESET}".rjust(TERMINAL_WIDTH * 2 // 3 - len(env_info)), flush=True)
        else:
            print(f"{GREEN}{env_info}{RESET}", f"{BOLD}{RED}[Failed]{RESET}".rjust(TERMINAL_WIDTH * 2 // 3 - len(env_info)), flush=True)
...
def run_task(app_path: str, env_path: str):
    """启动一个线程任务"""
    if app_to_envpaths.get(str(app_path)) is None or str(env_path) not in app_to_envpaths[str(app_path)]:
...
def task(env_path: Path):
    ...
    with max_threading_control:
        try:
            ...
            command = []
            exec_workspace = None
            if (env_path / KCL_FILE).is_file() and (ci_test_dir / SETTINGS_FILE).is_file():
...
def print_statistics(start_time: float):
    ...
    print(f"{BOLD}{GREEN}Total time: {spend_time:.2f}s, Total app num: {total_app_num}, Total env num: {total_env_num}, Time per env: {(spend_time/total_env_num):.2f}s{RESET}\n", flush=True)
    if len(failed_app_envs) == 0:
        sys.exit(0)
    else:
        sys.exit(1)
...
def validate_expected():
    try:
        KUSION_UPDATE_NOTICE = f"""{BOLD}{RED}WARN{RESET}: {RED}您的本地 kusion 版本需要升级，否则可能会导致本地编译失败
      请参考文档升级: https://yuque.antfin.com/docs/share/7de12ee1-07d0-4fa5-9330-56584fdaa75f?# 
      或者直接执行命令: wget http://antsys-kusion-dev.cn-hangzhou-alipay-b.oss-cdn.aliyun-inc.com/scripts/install_kusion.sh -qO - | bash{RESET}"""
        ...
        # Validate
        if  not (version.parse(min_kclvm_version) <= version.parse(current_kclvm_version)) or \
            not (version.parse(min_kcl_plugin_version) <= version.parse(current_kcl_plugin_version)):
            print(KUSION_UPDATE_NOTICE)
...
if __name__ == '__main__':
    ...
    for app_path_str in app_paths:
        ...
        if app_path.exists():
            # 如果目录存在，则当作应用目录或者环境目录启动任务
            run_task_by_app_path(app_path)
        else:
            # 如果目录不存在，则当作关键字搜索大库匹配的目录作为应用目录
            glob_root = utils.get_konfig_root()
            if app_path_str == "all":
                app_path_str = ""
            glob_paths = set(glob_root.glob(f"**/{app_path_str}")) # - set((glob_root/"base").glob("**"))
            glob_paths_str = list(set(list(map(lambda x: str(x), glob_paths))))
            print(f"{GREEN}{BOLD}Matched path{BOLD}{RESET}{GREEN}: {glob_paths_str}{RESET}", flush=True)
            print(f"{GREEN}{BOLD}Matched path total{BOLD}{RESET}{GREEN}: {len(glob_paths_str)}{RESET}\n", flush=True)
            glob_app_paths = []
    ...
    while True:
        # 所有应用都运行完毕，结束程序
        if all(map(check_app_finish, list(app_to_threads.keys()), list(app_to_threads.values()))):
            break
        time.sleep(1)
    # 输出统计信息
