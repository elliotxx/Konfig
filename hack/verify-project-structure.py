...
def test_project_structure(project_dir: str):
    ...
    for stack_dir, _, _ in os.walk(project_dir):
        ...
        assert (stack_dir / CI_TEST_DIR).is_dir(), f"file structure error: dir missing: {stack_name}/{CI_TEST_DIR}"
        assert (stack_dir / CI_TEST_DIR / SETTINGS_FILE).is_file(), f"file structure error: file missing: {stack_name}/{CI_TEST_DIR}/{STACK_FILE}"
        assert (stack_dir / CI_TEST_DIR / STDOUT_GOLDEN_FILE).is_file(), f"file structure error: file missing: {stack_name}/{CI_TEST_DIR}/{STDOUT_GOLDEN_FILE}"
...
def check_project_meta(project_dir: Path):
    ...
    assert yaml_content.get("name") is not None, "file structure error: invalid project meta: project name undefined"
    assert yaml_content.get("tenant") is not None, "file structure error: invalid project meta: tenant undefined"
...
def check_stack_meta(stack_dir: Path):
    ...
    if stack_dir.match("appops"):
        assert meta_env_name and meta_env_name.lower() == stack_dir.name.lower(), f"file structure error: invalid stack meta: (dir:{stack_dir.name}, meta:{meta_env_name})"
